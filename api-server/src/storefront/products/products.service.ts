...
export class StorefrontProductsService {
  ...
  private formatProduct(p: any) {
    ...
    const primaryImage =
      images.find((i: any) => i.is_primary) ?? images[0] ?? null;
  ...
  async listProducts(storeId: string, category_id?: string) {
    let categoryFilter: { categoryId: string | { in: string[] } } | undefined;
    if (category_id) {
      const children = await this.prisma.category.findMany({
        where: { parentId: category_id, storeId },
        select: { id: true },
      });
      categoryFilter =
        children.length > 0
    ...
    const products = await this.prisma.product.findMany({
      where: { storeId, isActive: true, ...categoryFilter },
